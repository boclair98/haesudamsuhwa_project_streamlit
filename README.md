...
1단 인입압력과 비에너지소비량(SEC)을 탐색하는 제조 운영 분석 프로토타입입니다.
...
RO Lens는 대규모 제조·유틸리티 조직의 운영자가 **읽기 전용으로 현황을 확인하고,
데이터와 모델의 신뢰 경계를 검토하는 첫 단계**를 목표로 합니다. 특정 기업의 도입이나
성능을 보증하는 제품은 아니며, 현장 시스템에 연결하기 전 다음 세 가지 게이트를 닫도록
구성했습니다.

게이트 | 현재 상태 | 운영 승격 조건
--- | --- | ---
데이터 계약 | 샘플 CSV 정제·중복 제거·단위 확인 | 센서 태그, 시간대, 결측·품질 플래그, 유량·회수율 계약
모델 검증 | 전체 기록 적합도와 모델 신호 공개 | 시간순 holdout, 현장 기준선, 드리프트·재학습 정책
통제·감사 | 읽기 전용 데모, 해시 검증 | SSO/RBAC, 감사 로그, shadow 운영 후 승인 절차
...
검토 큐 필터 | 압력·SEC 차이 조합과 표시 건수를 좁히고 필터 결과를 CSV로 내보내기
파일럿 개요 | 데이터 신뢰 게이트, 검토 큐, 현장 도입을 위한 다음 단계와 요약 CSV 제공
CSV 내보내기 | 월간 요약, 정제 기록, 검토 후보를 후속 분석용 CSV로 다운로드
데이터·모델 카드 | 데이터 출처, 결측·0값, 모델 입력 범위, 성능과 누락 변수를 공개
모델 설명 가능성 | 압력 계수와 SEC 상대 중요도를 시각화하되 인과효과로 해석하지 않도록 명시
...
- 회귀 지표와 백분위 계산
- 95백분위 기반 검토 후보 생성
- 압력 계수·SEC 상대 중요도 기반 모델 설명 가능성 테이블 생성

현재 검증 기준은 `unittest 9/9 passed`이며, 상세 내용은 [`VERIFICATION.md`](VERIFICATION.md)에 기록되어 있습니다.
...
✅ | 파일럿 개요, 도입 게이트, 모델 설명 가능성 추가
✅ | 데이터·모델 무결성 검증과 9개 자동 테스트 구성
